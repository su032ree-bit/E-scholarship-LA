...
if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['id'])) {
    ...
    // 2. ป้องกัน SQL Injection และรับค่า ID
    $news_id = mysqli_real_escape_string($connect1, $_GET['id']);

    // --- 3. ดึงข้อมูลข่าวจากฐานข้อมูลจริง ---
...
<head>
    ...
    <style>
        /* --- เพิ่มเติมเพื่อแก้ปัญหาตามสไตล์ admin.php และ news.php --- */
...
<body>
                ...
                <main class="main-content shadow-sm">
                    ...
                    <form id="newsForm" action="<?php echo $edit_mode ? '../news/update_news.php' : '../news/submit_news.php'; ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="d-flex justify-content-end gap-3 mt-5 pt-3 border-top">
                            ...
                            <button type="submit" id="btn-submit" class="btn btn-save" disabled>บันทึก</button>
                        </div>
                    </form>
                </main>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (menuHeader) {
                menuHeader.addEventListener('click', () => { if (window.innerWidth <= 1024) sidebar.classList.toggle('is-open'); });
            }

            // --- ส่วนตรวจสอบข้อมูลและ Disable ปุ่มบันทึก ---
            const titleInput = document.getElementById('news-title');
            const detailsInput = document.getElementById('news-details');
            const docNameInput = document.getElementById('news-file-name');
            ...
            const hasExistingFile = <?php echo ($edit_mode && !empty($news_file_actual)) ? 'true' : 'false'; ?>;

            function validateForm() {
                const currentTitle = titleInput.value.trim();
                const currentDetails = detailsInput.value.trim();
                const currentDocName = docNameInput.value.trim();
                const file = fileInput.files[0];
                
                let isFileValid = (file !== undefined || hasExistingFile);
                const isNotEmpty = (currentTitle !== "" && currentDetails !== "" && currentDocName !== "");
                
                let isChanged = true;
                if(isEditMode) {
                    isChanged = (currentTitle !== initialValues.title || currentDetails !== initialValues.details || currentDocName !== initialValues.docName || file !== undefined);
                }

                submitBtn.disabled = !(isNotEmpty && isFileValid && isChanged);
            }

            [titleInput, detailsInput, docNameInput].forEach(el => el.addEventListener('input', validateForm));
