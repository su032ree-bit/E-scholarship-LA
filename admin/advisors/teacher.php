...
if ($result_types && mysqli_num_rows($result_types) > 0) {
    ...
    if (!empty($data_types['st_name_1'])) $scholarship_types[1] = $data_types['st_name_1'];
    if (!empty($data_types['st_name_2'])) $scholarship_types[2] = $data_types['st_name_2'];
    if (!empty($data_types['st_name_3'])) $scholarship_types[3] = $data_types['st_name_3'];
}
...
if ($selected_scholarship_id > 0) {
    $sql_students = "SELECT s.st_id, s.st_firstname, s.st_lastname, sc.sco_id 
                 ...
                 WHERE s.st_type = '$selected_scholarship_id' AND s.st_activate = 1 
...
<body class="bg-light">
    ...
    <div class="teacher-page-wrapper">
        <!-- ส่วนกล่องขาวกึ่งกลางเป๊ะตามรูป 2 -->
        <div class="teacher-centered-card shadow-sm">
            ...
            <form action="teacher.php" method="get" id="scholarship-form">
                <select name="type" class="form-select teacher-dropdown-box" onchange="this.form.submit();">
