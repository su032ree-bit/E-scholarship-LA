...
<head>
    ...
    <style>
        /* --- แก้ไข Footer และลูกศรเมนู --- */
        body { display: flex; flex-direction: column; min-height: 100vh; margin: 0; }
        .dashboard-container { flex: 1 0 auto; }
                        ...
                        <tbody>
                            ...
                            if ($result && mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    $reporter_name = htmlspecialchars($row['st_firstname'] . ' ' . $row['st_lastname'] . ' (' . $row['st_code'] . ')');
                            ?>
                                    <tr>
                                        <td class="text-center fw-medium"><?php echo $i++; ?>.</td>
                                        <td class="fw-medium"><?php echo htmlspecialchars($row['issue_topic']); ?></td>
                                        <td class="fw-medium"><?php echo $reporter_name; ?></td>
                            ...
                            <?php
                                ...
                                echo '<tr><td colspan="5" class="text-center py-5 text-muted">ไม่มีรายการแจ้งปัญหาในระบบ</td></tr>';
                            }
                            ?>
                        </tbody>
                    </table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (notification) {
        setTimeout(() => { notification.style.opacity = '0'; setTimeout(() => { notification.style.display = 'none'; }, 500); }, 5000);
    }
