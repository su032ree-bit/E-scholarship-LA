...
// --- 1. คำนวณหา Base URL ของโปรเจกต์อัตโนมัติ ---
// ฟังก์ชันนี้จะช่วยให้เมนูลิงก์ไปถูกที่เสมอ ไม่ว่าจะอยู่ในโฟลเดอร์ admin/root หรือ admin/settings
$protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http";
$host = $_SERVER['HTTP_HOST'];
// หาตำแหน่งโฟลเดอร์ของโปรเจกต์ (สมมติว่าไฟล์นี้อยู่ในโปรเจกต์/admin/...)
$project_folder = str_replace(basename($_SERVER['PHP_SELF']), '', $_SERVER['PHP_SELF']);
// ตัดส่วนโฟลเดอร์ย่อยออกเพื่อให้ได้ Root ของเว็บไซต์จริงๆ
// หากโปรเจกต์คุณอยู่ที่ http://localhost/las/ ให้แน่ใจว่าได้ระบุเส้นทางที่ถูกต้อง
// วิธีง่ายที่สุดคือใช้ Path จาก Server Root (เริ่มต้นด้วย /) 
$base_path = "/scholar/"; // <--- ** สำคัญ: ถ้าชื่อโฟลเดอร์โปรเจกต์ไม่ใช่ "las" ให้เปลี่ยนเป็นชื่อโฟลเดอร์คุณ เช่น "/scholarship/" **

// --- 2. ดึงชื่อทุนและสถานะจาก tb_year ---
...
if (isset($connect1)) {
    ...
    if ($result_menu) {
        ...
        for ($i = 1; $i <= 3; $i++) {
            if (isset($data_menu["st_$i"]) && $data_menu["st_$i"] == 0) {
...
<style>
    ...
    /* ใช้ > li เพื่อควบคุมเฉพาะเมนูระดับแรก */
    ...
    /* --- Responsive Style (iPad/Mobile) --- */
    @media (max-width: 1024px) {
        .sidebar { position: static; width: 100%; margin-bottom: 20px; }
        .sidebar .menu-header { display: flex; justify-content: space-between; align-items: center; }
        .sidebar .menu-header::after { content: '\f107'; font-family: "Font Awesome 6 Free"; font-weight: 900; font-size: 14px; transition: 0.3s; }
        .sidebar.is-open .menu-header::after { content: '\f106'; }

        /* ซ่อนเฉพาะเมนูหลักในตอนแรก */
        .sidebar .menu-list > li { display: none; }
        .sidebar.is-open .menu-list > li { display: block; }

        /* เมนูย่อยในโหมด Responsive (Accordion) */
        .sidebar .menu-list .submenu {
            position: static;
            width: 100%;
            box-shadow: none;
        ...
        /* บังคับให้ li ภายในเมนูย่อยแสดงผลเสมอเมื่อ Submenu ถูกเปิด */
        .sidebar .menu-list li.has-submenu.submenu-open .submenu { display: block !important; }
        .sidebar .menu-list .submenu li { display: block !important; }
...
<aside class="sidebar">
    ...
    <ul class="menu-list">
        <!-- ลิงก์ทั้งหมดจะถูกเขียนแบบ Absolute Path จากโปรเจกต์รูท -->
        ...
        <li class="<?php if ($currentPage == 'academic_year.php') echo 'active'; ?>">
            <a href="<?php echo $base_path; ?>admin/settings/academic_year.php"> ข้อมูลปีการศึกษา</a>
        </li>
        ...
        <li class="<?php if (in_array($currentPage, ['susp_std.php', 'susp_std_add.php'])) echo 'active'; ?>">
            <!-- ย้ายไปโฟลเดอร์ student ที่อยู่นอก admin -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    hasSubmenuItems.forEach(item => {
        ...
        link.addEventListener('click', function(e) {
            if (window.innerWidth <= 1024) {
                e.preventDefault();
                e.stopPropagation(); // ป้องกัน Event ไหลไปที่ Sidebar หลัก
